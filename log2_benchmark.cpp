...
template <typename T> void log2_benchmark(boost::random::mt19937 rng, int N)
{
...
#define clock chrono::steady_clock
  ...
  cerr << "Generating random "<<Nbits<<"-bit numbers... ";
  vector<T> random_T(N);
  for(int i=0;i<N;i++)
    random_T[i] = rand_elem(rng);
  auto  end0  = clock::now();
  ...
  for(int i=0;i<N;i++)
    msb = floor_log2_slow(random_T[i]);
  auto  end1  = clock::now();
...
int main()
{
  ...
  log2_benchmark<uint8_t> (rng,N);
  log2_benchmark<uint16_t>(rng,N);
  log2_benchmark<uint32_t>(rng,N);
  log2_benchmark<uint64_t>(rng,N);    
